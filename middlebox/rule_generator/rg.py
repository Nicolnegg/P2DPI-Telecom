...
def obfuscate_rule_fkh(pattern: str, kmb_hex_str: str) -> str:
    ...
    res = prf.FKH_hex(c_char_p(key_str), c_char_p(kmb_hex_bytes), output_buffer, 200)
    if res != 1:
...
def generate_and_send_rules(mb_url="http://localhost:9999/upload_rules"):
    ...
    for rule in RULES:
        ...
        obf_rule = obfuscate_rule_fkh(rule["pattern"], K_MB_HEX)
